...
export default async function handler(req, res) {
  ...
  req.on("end", async () => {
    ...
    if (!id_workspace) {
      return res.status(400).json({ error: "Parameter 'id_workspace' is required." });
    }
    ...
    if (courseError) {
      return res.status(500).json({ error: `Failed to delete related courses: ${courseError.message}` });
    }
    ...
    if (workspaceError) {
      return res.status(500).json({ error: `Failed to delete workspace: ${workspaceError.message}` });
    }
